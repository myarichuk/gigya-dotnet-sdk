...
namespace Gigya.Socialize.SDK
{
    ...
    {
        private readonly static ArrayPool<byte> TempArrayPool = ArrayPool<byte>.Create();
        private readonly static ThreadLocal<HMAC> EncryptionAlogirthm = new ThreadLocal<HMAC>(() =>
        {
        ...
        public static bool ValidateUserSignature(string UID, string timestamp, string secret, string signature, int expiration) =>
            !SignatureTimestampExpired(timestamp, expiration) && ValidateUserSignature(UID, timestamp, secret, signature);
        ...
        private static int GetOriginalLengthInBytes(ReadOnlySpan<char> base64string)
        {
            if (base64string.IsEmpty)
                return 0;
            var characterCount = base64string.Length;
            var possiblePadding = base64string.Slice(characterCount - 2, 2);
            var paddingCount = 0;
            for (int i = 0; i < possiblePadding.Length; i++)
                if (possiblePadding[i] == '=')
                    paddingCount++;
        ...
        /// <returns></returns>
        public unsafe static string CalcSignature(string text, string key)
        {
#if NETSTANDARD2_0
            ...
            var dataBuffer = stackalloc byte[byteCount];
            int keyBufferLength = GetOriginalLengthInBytes(key);
            var keyBuffer = stackalloc byte[keyBufferLength];

            var data = new Span<byte>(dataBuffer, byteCount);
            var keyData = new Span<byte>(keyBuffer, keyBufferLength);

            Encoding.UTF8.GetBytes(text.AsSpan(), data);

            if (!Convert.TryFromBase64String(key, keyData, out _))
            ...
            try
            {
                ...
                keyBytes = TempArrayPool.Rent(keyBufferLength);
                keyData.CopyTo(keyBytes);
                EncryptionAlogirthm.Value.Key = keyBytes;

                //HMAC SHA1 is always 20 bytes long
                var hashBuffer = stackalloc byte[20];
                var hash = new Span<byte>(hashBuffer, 20);
                if (!EncryptionAlogirthm.Value.TryComputeHash(data, hash, out _))
                    throw new InvalidOperationException("Failed to compute hash, this is not supposed to happen!");
                return Convert.ToBase64String(hash);
            }
            finally
            {
                if (keyBytes != null)
                    TempArrayPool.Return(keyBytes);
            }
#endif
        }
        ...
        /// <param name="requestParams">list of params of the request</param>
        /// <returns>the base string to act on for calculating the OAuth1 cryptographic signature</returns>
        public static string CalcOAuth1Basestring(string httpMethod, string url, IDictionary<string, string> requestParams)
        {
            //TODO: refactor this, as the implementation is far from ideal
            ...
            foreach (var kvp in requestParams)
            {
                if (kvp.Key != null)
                {
                    ...
                    querystring.Append("=");
                    querystring.Append(HttpUtility.UrlEncode(kvp.Value));
                    querystring.Append("&");
                }
            }
            ...
            string basestring =$"{httpMethod.ToUpperInvariant()}&{HttpUtility.UrlEncode(normalizedUrlSB.ToString())}&{HttpUtility.UrlEncode(querystring.ToString())}";
            return basestring;
        ...
        internal static int CurrentTimeSeconds() => (int)(DateTime.UtcNow - new DateTime(1970, 1, 1)).TotalSeconds;
